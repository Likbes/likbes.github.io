class ConnectionManager {
    ...
    updateManager(peers) {
        ...
        const client = clients[0].state.player;
        const local = this.tetrisManager.instances[0];
        const anotherTetris = this.tetrisManager.instances[1];
        anotherTetris.shadow.matrix = client.matrix;
    ...
    updatePeer(id, fragment, [key, value]) {
        ...
        if (fragment === 'arena') {
            tetris.shadow.arenaMatrix = value;
        }

        if (key === 'score') {
            tetris.updateScore(value);
        } else if (key === 'matrix') {
            tetris.shadow.matrix = tetris.player.matrix;
            tetris.draw();
        } else {
            tetris.draw();
        }
